...
@OnlyIn(Dist.CLIENT)
public class CustomChatGui extends NewChatGui {
    ...
    public CustomChatGui(Minecraft p_i1022_1_) {
        super(p_i1022_1_);
    ...
    public void render(MatrixStack p_238492_1_, int p_238492_2_) {
        if (!this.isChatHidden()) {
            ...
            if (j > 0) {
                ...
                int k = MathHelper.ceil((double)this.getWidth() / scale);
                ...
                // 文字の背景の透明度
                // double bgOpacity = this.minecraft.options.textBackgroundOpacity;
                double textOpacity = 0.5;
                // チャットの一段ごとの隙間
                // double lineSpace = 9.0D * (this.minecraft.options.chatLineSpacing + 1.0D);
                ...
                for (int i = 0; i + this.chatScrollbarPos < this.trimmedMessages.size() && i < linesPerPage; i++) {
                    ChatLine<IReorderingProcessor> chatline = this.trimmedMessages.get(i + this.chatScrollbarPos);
                    if (chatline != null) {
                        int j1 = p_238492_2_ - chatline.getAddedTime();
                        if (j1 < 200 || isFocused) {
                            double d5 = isFocused ? 1.0D : getTimeFactor(j1);
                            int l1 = (int)(255.0D * d5 * chatOpacity);
                            int i2 = (int)(255.0D * d5 * textOpacity);
                            ++l;
                            if (l1 > 3) {
                                int j2 = 0;
                                ...
                                fill(p_238492_1_, -2, (int)(d6 - d3), k + 4, (int)d6, i2 << 24);
                ...
                if (true) {
                    ...
                    int i3 = (int)(255.0D * textOpacity);
                    p_238492_1_.pushPose();
                    p_238492_1_.translate(0.0D, 0.0D, 50.0D);
                    fill(p_238492_1_, -2, 0, k + 4, 9, i3 << 24);
                    RenderSystem.enableBlend();
                    p_238492_1_.translate(0.0D, 0.0D, 50.0D);
                    this.minecraft.font.drawShadow(p_238492_1_, new TranslationTextComponent("chat.queue", this.chatQueue.size()), 0.0F, 1.0F, 0xffffff + (k2 << 24));
                    p_238492_1_.popPose();
                    RenderSystem.disableAlphaTest();
                    RenderSystem.disableBlend();
                }
